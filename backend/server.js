...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJIUzI1NiIsInJlZiI6Imp1bHBoZXV1bW9sbndrdGhhemRqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQwMzc5ODYsImV4cCI6MjA4OTYxMzk4Nn0.i3jI-PjdAUPnbgVn_EXctr0-F158Gbp-r6icrEdvOGM';
const SUPABASE_SERVICE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJIUzI1NiIsInJlZiI6Imp1bHBoZXV1bW9sbndrdGhhemRqIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3NDAzNzk4NiwiZXhwIjoyMDg5NjEzOTg2fQ.eCDb9XAD02Pi8Ejl3vIz9ROJC80zBpAOW4svI7M9GR4';
...
function buildAskAllieJobContext(jobContext = {}) {
  return {
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: mode === 'expert' ? expertSystemPrompt : standardSystemPrompt },
...
function buildAskAllieFallbackAnswer(extracted, externalResearch) {
  const pinoutTable = Array.isArray(extracted?.pinout_table) ? extracted.pinout_table.map(pin => ({
    pin_label: safeString(pin.pin || pin.pin_label),
    wire_color: safeString(pin.wire_color),
    circuit_function: safeString(pin.function || pin.circuit_function),
    expected_value: safeString(pin.expected_voltage || pin.expected_value),
...
app.get('/admin-users', async (req, res) => {
  try {
    ...
    if (error) {
      return res.json({ success: false, error: error.message, users: [] });
    }
...
app.get('/user-profile/:id', async (req, res) => {
  try {
    ...
    if (!userId) {
      return res.json({ success: false, error: 'User id required', profile: null });
    }
    ...
    if (error) {
      return res.json({ success: false, error: error.message, profile: null });
    }

    return res.json({ success: true, profile: data });
  } catch (err) {
    return res.json({ success: false, error: err.message, profile: null });
  }
});
...
app.get('/access-requests', async (req, res) => {
  try {
    ...
    if (error) {
      return res.json({ success: false, error: error.message, requests: [] });
    }

    return res.json({ success: true, requests: data || [] });
  } catch (err) {
    return res.json({ success: false, error: err.message, requests: [] });
  }
});
...
app.post('/approve-request', async (req, res) => {
  try {
    ...
    if (!requestId) {
      return res.json({ success: false, error: 'request_id is required' });
    }
...
app.post('/chat', async (req, res) => {
  try {
    ...
    if (action === 'make_next_step') {
      ...
      return res.json({ success: true, reply, next_step: nextStep });
    }
...
app.post('/ask-allie', async (req, res) => {
  try {
    ...
    const jobContext = buildAskAllieJobContext(req.body.job_context || {});
    ...
    if (!session) {
      const sessionPayload = {
        ...
        prior_tests: Array.isArray(jobContext.prior_tests) ? jobContext.prior_tests : [],
...
app.post('/ask-allie-feedback', async (req, res) => {
  try {
    ...
    const vehicleContext = buildAskAllieJobContext(req.body.vehicle_context || {});

    if (!sessionId || !feedbackType) {
      return res.status(400).json({ success: false, error: 'session_id and feedback_type are required' });
    }
    ...
    return res.json({ success: true, message: 'Ask Allie feedback saved' });
  } catch (err) {
    return res.status(500).json({ success: false, error: err.message });
  }
});
...
app.post('/signup', async (req, res) => {
  try {
    ...
    if (requestError) {
      return res.json({ success: false, error: requestError.message });
    }
